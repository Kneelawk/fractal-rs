...
pub fn build_four_points_offsets(offset: f32) -> Vec<(f32, f32)> {
    vec![
        (
            0.5 - offset,
            0.5 - offset,
...
pub fn build_linear_offsets(axial_points: u32) -> Vec<(f32, f32)> {
    let mut vec = vec![];
    ...
    for y in 0..axial_points {
        for x in 0..axial_points {
            vec.push((
                x as f32 * offset + initial_offset,
                y as f32 * offset + initial_offset,
            ))
        }
    }
